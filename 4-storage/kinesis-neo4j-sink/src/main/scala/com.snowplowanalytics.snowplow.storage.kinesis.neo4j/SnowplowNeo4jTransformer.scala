...
// Amazon
import com.amazonaws.services.kinesis.connectors.interfaces.{ITransformerBase, ITransformer}
 ...
 */
class SnowplowNeo4jTransformer(documentIndex: String, documentType: String) extends ITransformerBase[JsonRecord, Neo4jObject]
  ...
  def jsonifyGoodEvent(event: Array[String]): JsonRecord = {
    ...
    JsonRecord(compact(render(eventJson)), ScalazJson4sUtils.extract[String](eventJson, "event_id").toOption, Some(eventJson))
  }
  ...
  override def fromClass(record: JsonRecord): Neo4jObject =
    record.id match {
      case Some(eid) => Neo4jObject(documentIndex, documentType, record.jObject.map(Right(_)).getOrElse(Left(record.json)), Some(eid))
      case None => Neo4jObject(documentIndex, documentType, Left(record.toString))
    }
