...
object Neo4jSinkApp extends App {
  ...
  val executor = configValue.getString("source") match {
    case "kinesis" => new Neo4jSinkExecutor(streamType, documentIndex, documentType, convertConfig(configValue))
    case "stdin" => new Runnable {
      val transformer = new SnowplowNeo4jTransformer(documentIndex, documentType)
  ...
  def convertConfig(connector: Config): KinesisConnectorConfiguration = {
    ...
    props.setProperty(KinesisConnectorConfiguration.PROP_CONNECTOR_DESTINATION, "neo4j")
